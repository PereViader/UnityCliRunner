    ...
    internal static class OperationExecutionEngine
    {
        ...
        public static bool TryCancel(string operationId)
        {
            lock (s_CtsLock)
            {
                if (s_ActiveCts != null && s_ActiveIsCancelable && (string.IsNullOrEmpty(operationId) || s_ActiveOperationId == operationId))
                {
                    ...
                    {
                        Debug.LogWarning($"UnityCliRunner: Failed to cancel active operation: {ex.Message}");
                    }
                }
            }
            return false;
        ...
        public static void MarkInterrupted(string operationKind, string resultFilePath, string message, string targetOperationId = null)
        {
            ...
            try
            {
                ...
                UnityCliOperationStore.WriteAtomic(resultFilePath, JsonUtility.ToJson(result, true), opId);
            }
            catch (Exception ex)
            {
                Debug.LogError($"UnityCliRunner: Failed to persist interrupted {operationKind} result: {ex}");
            }
            finally
            {
            ...
            Action<Exception> onInvocationException = null)
        {
            ...
            catch (TargetInvocationException tie)
            {
                ...
                {
                    string cancelMsg = GetCancellationMessage(operationKind);
                    Debug.LogWarning($"UnityCliRunner: {operationKind} execution was canceled: {inner.Message}");
            ...
            catch (OperationCanceledException oce)
            {
                stopwatch.Stop();
                string cancelMsg = GetCancellationMessage(operationKind);
                Debug.LogWarning($"UnityCliRunner: {operationKind} execution was canceled: {oce.Message}");
                var logs = logCapture.GetLogs();
                DisposeCapture(logCapture);
                FinishOperation(operationId, operationKind, resultFilePath, false, cancelMsg, stopwatch.Elapsed.TotalSeconds, null, logs, interrupted: true);
                return;
            ...
            Action<Exception> onInvocationException)
        {
            if (innerTask.IsFaulted)
            {
                ...
                {
                    string cancelMsg = GetCancellationMessage(operationKind);
                    Debug.LogWarning($"UnityCliRunner: {operationKind} execution was canceled: {ex.Message}");
            ...
            if (innerTask.IsCanceled)
            {
                ...
                DisposeCapture(logCapture);
                string cancelMsg = GetCancellationMessage(operationKind);
                Debug.LogWarning($"UnityCliRunner: {operationKind} execution was canceled.");
                FinishOperation(operationId, operationKind, resultFilePath, false, cancelMsg, duration, null, logs, interrupted: true);
                return;
            }
            ...
            bool interrupted = false)
        {
            ...
            if (!UnityCliOperationStore.IsOwnedBy(operationId, operationKind))
            {
                return;
            }
            ...
            try
            {
                ...
                UnityCliOperationStore.WriteAtomic(resultFilePath, json, operationId);
            }
            catch (Exception ex)
            {
                Debug.LogError($"UnityCliRunner: Failed to write {operationKind} result: {ex}");
            }
            finally
            {
